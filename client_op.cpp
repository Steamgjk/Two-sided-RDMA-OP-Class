...
void client_op::build_connection(struct rdma_cm_id *id)
{
...
void client_op::build_context(struct ibv_context *verbs)
{
	if (s_ctx)
	{
		if (s_ctx->ctx != verbs)
...
void client_op::build_params(struct rdma_conn_param *params)
{
...
void client_op::build_qp_attr(struct ibv_qp_init_attr *qp_attr)
{
...
void client_op::event_loop(struct rdma_event_channel *ec, int exit_on_disconnect)
{
...
void * client_op::poll_cq(void *ctx)
{
	struct ibv_cq *cq;
	struct ibv_wc wc;
...
void client_op::rc_server_loop(const char *port)
{
...
void client_op::rc_disconnect(struct rdma_cm_id *id)
{
	rdma_disconnect(id);
}

void client_op::rc_die(const char *reason)
{
	fprintf(stderr, "%s\n", reason);
	exit(EXIT_FAILURE);
}

struct ibv_pd * client_op::rc_get_pd()
{
	return s_ctx->pd;
}